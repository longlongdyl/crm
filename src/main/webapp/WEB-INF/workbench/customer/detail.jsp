...
<body>
	...
	<div style="position: relative; top: 40px; left: 40px;">
		...
		<!-- 备注1 -->
		<c:forEach items="${customer.customerRemarkList}" var="customerRemarkList">
			<div class="remarkDiv" style="height: 60px;" id="remarkDiv${customerRemarkList.id}">
				<img title="${clue.owner}" src="/crm/image/user-thumbnail.png" style="width: 30px; height:30px;">
				<div style="position: relative; top: -40px; left: 40px;">
					<h5 id="${customerRemarkList.id}">${customerRemarkList.noteContent}</h5>
					<font color="gray">线索</font> <font color="gray">-</font> <b>${customer.owner}-${customer.name}</b>
					<small style="color: gray;">${customerRemarkList.createTime} 由${customerRemarkList.createBy}</small>
					<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">
						<a class="myHref" href="javascript:void(0);"><span onclick="updateCustomerRemarkList('${customerRemarkList.id}')"class="glyphicon glyphicon-edit" style="font-size: 20px; color: #E6E6E6;"></span></a>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<a class="myHref" href="javascript:void(0);"><span onclick="deleteCustomerRemark('${customerRemarkList.id}')" class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>
					</div>
				</div>
			</div>
		</c:forEach>
					...
					<div class="modal-body">
						<form class="form-horizontal" role="form">
							<div class="form-group">
								...
								<div class="col-sm-10" style="width: 81%;">
									<textarea class="form-control" rows="3" id="noteContent">123</textarea>
								</div>
							</div>
						</form>
					</div>
		...
		<div id="remarkDiv" style="background-color: #E6E6E6; width: 870px; height: 90px;">
			<form role="form" style="position: relative;top: 10px; left: 10px;">
				...
				<p id="cancelAndSaveBtn" style="position: relative;left: 737px; top: 10px; display: none;">
					...
					<button type="button" class="btn btn-primary" onclick="addCustomerRemark()" id="saveCustomerRemarkBtn">保存</button>
				</p>
			</form>
		</div>
	</div>
...
<script>
	...
	function deleteCustomerRemark(id) {
		if (confirm("确认删除此条备注?")) {
			$.ajax({
				url:'/crm/workbench/customer/deleteCustomerRemark',
				data:{'id':id},
				type:'post',
				dataType:'json',
	...
	function updateCustomerRemarkList(id) {
	...
	function addCustomerRemark() {
		...
		$.ajax({
			url: '/crm/workbench/customer/addCustomerRemark',
			data: {
			...
			success: function (data) {
				...
				$('#div1').prepend("<link href=\"/crm/jquery/bootstrap_3.3.0/css/bootstrap.min.css\" type=\"text/css\" rel=\"stylesheet\" />" +
						"\t\t\t <div class=\"remarkDiv\" style=\"height: 60px;\" id=\"remarkDiv"+x+"\">" +
						"\t\t\t\t <img title=\"${customer.owner}\" src=\"../../image/user-thumbnail.png\" style=\"width: 30px; height:30px;\">\n" +
						"\n" +
						"\t\t\t\t <div style=\"position: relative; top: -40px; left: 40px;\" >\n" +
						"\t\t\t\t\t <h5 id="+x+">" + $('#remark').val() + "</h5>\n" +
						"\t\t\t\t\t <font color=\"gray\">线索</font> <font color=\"gray\">-</font> <b>${customer.owner}-${customer.name}</b> <small style=\"color: gray;\">" + onedate + "由${customer.createBy}</small>\n" +
						"                <div style=\"position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;\">\n" +
						" <a class='myHref' href='javascript:void(0);'><span id=z"+x+" class='glyphicon glyphicon-edit' style='font-size: 20px; color: #E6E6E6;'" +
						"    onclick="+"updateCustomerRemarkList('"+x+"') ></span></a>" +
						"                    &nbsp;&nbsp;&nbsp;&nbsp;\n" +
						"                    <a class=\"myHref\" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-remove\" style=\"font-size: 20px; color: #E6E6E6;" +
						"    onclick="+"deleteCustomerRemark('"+x+"')   ></span></a>\n" +
						"                </div>"+
						"\t\t\t\t </div>\n" +
						"\t\t\t </div>");
				$('#remark').val('');
	...
	$('#updateRemarkBtn').click(function () {
		...
		var id = $('#remarkId').val()
		$.ajax({
			url: '/crm/workbench/customer/updateCustomerRemark',
			data: {
