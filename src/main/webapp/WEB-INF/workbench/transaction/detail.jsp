...
<div style="position: relative; left: 40px; top: -30px;">
    ...
    <div style="position: relative; height: 50px; width: 250px;  top: -72px; left: 700px;">
        <button type="button" class="btn btn-default"
                onclick="window.location.href='/crm/workbench/transaction/queryTranById?id='+'${tran.id}'+'&edit=1';">
    ...
    <c:forEach items="${caChes}" var="caChe">
        <c:if test="${caChe.code == 'stage'}">
            ...
            <c:forEach items="${caChe.caCheValueList}" var="caCheValue">
                <%j++;%>
                <c:if test="${caCheValue.value < tran.stage && tran.stage <'08丢失的线索'}">
                    <span class="glyphicon glyphicon-ok-circle mystage" data-toggle="popover"
                          onclick="changeStage('${caCheValue.value}')" data-placement="bottom"
                          data-content="${caCheValue.value}" style="color: #90F790;"></span>
                    -----------
                </c:if>
                ...
                <c:if test="${caCheValue.value > tran.stage && tran.stage <'08丢失的线索'}">
                    <%if (j<8){%>
                        <span class="glyphicon glyphicon-record mystage" data-toggle="popover"
                        onclick="changeStage('${caCheValue.value}')" data-placement="bottom"
                        data-content="${caCheValue.value}"></span>
                                -----------
                    <%}%>
                    <%if (j>=8){%>
                ...
                <c:if test="${tran.stage == '08丢失的线索'}">
                    ...
                    <span class="glyphicon glyphicon-remove mystage" data-toggle="popover"
                          onclick="changeStage('${caCheValue.value}')" data-placement="bottom"
                          data-content="${caCheValue.value}" style="color: red;"></span>
                    -----------
                    <%}%>
                    <% if (i > 8) {%>
                    <span class="glyphicon glyphicon-remove mystage" data-toggle="popover"
                          onclick="changeStage('${caCheValue.value}')" data-placement="bottom"
                          data-content="${caCheValue.value}" style="color: black;"></span>
                    -----------
                    <%}%>
                </c:if>
                <c:if test="${tran.stage == '09因竞争丢失关闭'}">
                    <% i++; %>
                    <% if (i < 8) {%>
                    <span class="glyphicon glyphicon-record mystage" data-toggle="popover"
                          onclick="changeStage('${caCheValue.value}')" data-placement="bottom"
    ...
    <div style="position: relative; top: 100px; left: 40px;">
        ...
        <div style="position: relative;top: 0px;">
            <table id="activityTable" class="table table-hover" style="width: 900px;">
                ...
                <tbody>
                <c:forEach items="${tran.historyList}" var="historyList">
